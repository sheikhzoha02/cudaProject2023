...
def save_checkpoint(model, optimizer, epoch, mean_iou_sequential, mean_accuracy_sequential,
                    mean_iou_augmented, mean_accuracy_augmented, mean_loss_sequential, mean_loss_augmented,
                    train_mean_iou_sequential, train_mean_accuracy_sequential, train_mean_iou_augmented,
                    train_mean_accuracy_augmented,
                    train_mean_loss_sequential, train_mean_loss_augmented, checkpoint_path):
...
def train(generator, img_batch, img_data, img_batch_val, epochs, lr, validation_interval, augmented, num_classes=20):
    ...
    if not os.path.exists('./final_training_resnet/'):
        os.makedirs('./final_training_resnet/')

    if not os.path.exists('./models_resnet/'):
        os.makedirs('./models_resnet/')

    if not os.path.exists('./final_gif_images_resnet/'):
        os.makedirs('./final_gif_images_resnet/')
    ...
    for epoch in range(epochs):
        ...
        mean_iou_sequential, mean_accuracy_sequential, mean_iou_augmented, mean_accuracy_augmented, mean_loss_sequential, mean_loss_augmented = validate(
            generator, img_batch_val, img_data, epoch, augmented, num_classes=20)
        save_checkpoint(generator, gen_optimizer, epoch, mean_iou_sequential, mean_accuracy_sequential,
                        mean_iou_augmented, mean_accuracy_augmented, mean_loss_sequential, mean_loss_augmented,
                        train_mean_iou_sequential, train_mean_accuracy_sequential, train_mean_iou_augmented,
                        train_mean_accuracy_augmented, train_mean_loss_sequential, train_mean_loss_augmented,
                        f'models_resnet/{epoch}_{idx_batch}.pth')
...
def validate(generator, img_batch_val, img_data, epoch, augmented, num_classes=20):
    ...
    with torch.no_grad():
        for idx_batch, (sequential_frames, skip_frames) in enumerate(img_batch_val):
            ...
            y_, y, x = iterate_validation_images(generator, sequential_frames[0], sequential_frames[1],
                                                 sequential_frames[2])
            for result in y:
            ...
            [v_utils.save_image(i, f"final_training_resnet/generated_sequential_{epoch}_{ind}.png") for ind, i in
             enumerate(generated_image)]
            save_tensors_as_gif(gif_images_sequential, f"final_gif_images_resnet/generated_sequential_{epoch}.gif", 0.1)

            # gif_images_sequential = [(torch.tensor(i).squeeze().cpu()) for i in x]
            [v_utils.save_image(i, f"final_training_resnet/original_sequential_{epoch}_{ind}.png") for ind, i in
             enumerate(x)]
            # save_tensors_as_gif(gif_images_sequential,f"final_gif_images_resnet/original_sequential_{epoch}.gif",0.1)
            ...
            if (augmented == True):
                ...
                generated_image = [img_data.decode_segmap(np.squeeze(i.data.max(1)[1][0, :, :].cpu().numpy())) for i in
                                   y]
                generated_image = [decode_segmap_to_rgb(i) for i in generated_image]
                gif_images_augmented = [(torch.tensor(i)) for i in generated_image]
                [v_utils.save_image(i, f"final_training_resnet/generated_augmented_{epoch}_{ind}.png") for ind, i in
                 enumerate(generated_image)]
                save_tensors_as_gif(gif_images_augmented, f"final_gif_images_resnet/generated_augmented_{epoch}.gif",
                                    0.1)

                # gif_images_augmented = [(torch.tensor(i).squeeze().cpu()) for i in x]
                [v_utils.save_image(i, f"final_training_resnet/original_augmented_{epoch}_{ind}.png") for ind, i in
                 enumerate(x)]
                # save_tensors_as_gif(gif_images_augmented,f"final_gif_images_resnet/original_augmented_{epoch}.gif",0.1)
    ...
    return mean_iou_sequential, mean_accuracy_sequential, mean_iou_augmented, mean_accuracy_augmented, mean_loss_sequential, mean_loss_augmented
